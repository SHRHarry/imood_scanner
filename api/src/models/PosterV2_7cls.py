...
# from .mix_transformer import MixVisionTransformer, OverlapPatchEmbed
...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768):
        ...
        face_landback_checkpoint = torch.load(r'./api/src/models/pretrain/mobilefacenet_model_best.pth.tar',
        ...
        # self.MIT = MixVisionTransformer(embed_dims=[160, 160], num_heads=[8, 8], mlp_ratios=[4, 4],
        #                                 patch_size=14, depths=[1, 1], num_classes=num_classes, qkv_bias=True,
        #                                 norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[2, 1], drop_rate=0.0, drop_path_rate=0.1)


        self.ir_back = Backbone(50, 0.0, 'ir')
        ir_checkpoint = torch.load(r'./api/src/models/pretrain/ir50.pth', map_location=lambda storage, loc: storage)
        ...
        # self.embed_v = OverlapPatchEmbed(img_size=14, patch_size=3, stride=1, in_chans=256, embed_dim=768)
    ...
    def forward(self, x):
        ...
        # print(f"[channel_first] o1.shape = {o1.shape}, o2.shape = {o2.shape}, o3.shape = {o3.shape}")

        o1, o2, o3 = self.embed_q(o1).flatten(2).transpose(1, 2), self.embed_k(o2).flatten(2).transpose(1, 2), self.embed_v(o3)
        # o1, o2 = self.embed_q(o1).flatten(2).transpose(1, 2), self.embed_k(o2).flatten(2).transpose(1, 2)
        # o3, H, W = self.embed_v(o3)

        # print(f"[embed] o1.shape = {o1.shape}, o2.shape = {o2.shape}, o3.shape = {o3.shape}")
        o = torch.cat([o1, o2, o3], dim=1)
        # print(f"[cat] o.shape = {o.shape}")
